...
-   Key highlights (coordinate them with Juwon):
    -   Within-city variation, fall phenology: [Katz et al., 2019](https://www.sciencedirect.com/science/article/pii/S0048969718343675) [Xing et al., 2022](https://link.springer.com/article/10.1007/s00484-022-02322-1). A few papers about intra-urban variation of phenology. Few about the fall phenology.
...
Using `SOS` as an example, I fitted a linear mixed-effects model for each genus with environmental variables (`AvgTemp_mean`, `Sum_mm_sum`, and `srad_mean`) as predictors, with random effects for `year`. I used `check_heteroscedasticity()` to check for heteroscedasticity in the residuals. The results showed almost every model had heterogenerous residuals.
...
```{r}
#| label: fig-direct_map
#| include: true
#| warning: false
#| fig-cap: "SOS, Acer as example"

# tavg_allyear_var_2020 <- readRDS("~/phenology-urban/data/proc/urban/NY/tavg_allyear_var.rds") %>%
#   filter(genus == "Acer" & direction == "up" & thres == 0.5 & year== 2020) %>%
#   na.omit() %>%
#   group_by(id)%>%
#   slice(1) %>%
#   ungroup() %>%
...
#   geom_sf(data = tavg_allyear_var_2019, aes(color = AvgTemp_mean), size = 0.5) +
#   scale_color_viridis_c(option = "C") +
#   theme_minimal()
# 
# ggplot() +
#   geom_sf(data = boundary, fill = "grey90") +
#   geom_sf(data = tavg_allyear_var_2020, aes(color = AvgTemp_mean), size = 0.5) +
#   scale_color_viridis_c(option = "C") +
#   theme_minimal()
# 
# lm_result <- lm(doy ~ AvgTemp_mean, data = tavg_allyear_var)
# # Extract slope and significance
# slope <- coef(lm_result)[2] # Slope is the second coefficient
# p_value <- summary(lm_result)$coefficients[2, 4] # P-value for the slope
# 
# # Add text annotation for slope and p-value
# ggplot(tavg_allyear_var, aes(x = AvgTemp_mean, y = doy, color = genus)) +
#   geom_point(size = 1) +
#   geom_smooth(method = "lm", se = TRUE, aes(group = genus), linetype = "dashed") +
#   annotate("text", x = mean(tavg_allyear_var$AvgTemp_mean), y = max(tavg_allyear_var$doy),
...
I modeled the relationship between the phenology and environmental covariates using a spatial hierarchical model. The response variable (phenology parameter) at location $\mathbf{s}_i$, denoted as $y(\mathbf{s}_i)$, was modeled as follows:
...
Details on handling `year` (replicate location):

-   [x] Treat `year` as random effect

Details on handling `genus`:
...
#| fig-width: 8
#| fig-cap: 
#|   - "The distribution of the posterior of regression coefficients for `Temp~avg~` for each genus in New York city (Thick line: 90% CI, thin line: 95% CI, point: mean value)."
#|   - "The distribution of the posterior of regression coefficients for `Temp~avg~` for each genus in New York city (Line: 95% CI, point: mean value)."
#| warning: false
#| eval: false
